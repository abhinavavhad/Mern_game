...
export const initializeDB = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error initializing database", error: err.message });
  }
};
  ...
  try {
    ...
    const results = await Promise.all(
      priceRanges.map(async (range) => {
        ...
        return {
          range: `${range.min} - ${
            range.max === Infinity ? "above 901" : range.max
          }`,
          count,
        };
      })
...
export const getcombinedAPI = async (req, res) => {
  ...
  try {
    ...
    const statistics = {
      totalSaleAmount: totalSaleAmountData.length
        ? totalSaleAmountData[0].totalAmount
        : 0,
      totalSoldItems,
      totalNotSoldItems,
    };
